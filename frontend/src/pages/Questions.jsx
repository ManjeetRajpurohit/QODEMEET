  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.post(
        import.meta.env.VITE_BACKEND_URL + `/api/question/remove/${id}`,
        {},
        { headers: { token } },
      );
    ...
    <div className="min-h-screen bg-[#050816] p-6 text-white">
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-400">Manage interview questions</p>
        </div>

        <button
          onClick={() => navigate("/dashboard/addquestion")}
          className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 font-medium transition hover:bg-indigo-700"
        >
          <PlusCircle size={18} />
          Add Question
        </button>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
          ...
          <h2 className="mt-2 text-3xl font-bold">{questions.length}</h2>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
          ...
          <tbody>
            {filteredQuestions.map((question) => (
              ...
              >
                ...
                <td className="px-6 py-5 text-gray-300">{question.category}</td>
        ...
        {filteredQuestions.length === 0 && (
          <div className="py-16 text-center">
            <FileText size={50} className="mx-auto mb-4 text-gray-500" />
